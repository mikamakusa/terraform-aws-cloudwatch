...
resource "aws_cloudwatch_metric_stream" "this" {
  ...
  dynamic "statistics_configuration" {
    ...
    content {
      additional_statistics = lookup(statistics_configuration.value, "additional_statistics")
      dynamic "include_metric" {
        for_each = lookup(statistics_configuration.value, "include_metric")
        content {
...
resource "aws_resourcegroups_group" "this" {
  ...
  tags = merge(
    var.tags,
    lookup(var.resourcegroups_group[count.index], "tags"),
    data.aws_default_tags.this.tags
  )
...
resource "aws_applicationinsights_application" "this" {
  ...
  tags = merge(
    var.tags,
    lookup(var.applicationinsights[count.index], "tags"),
    data.aws_default_tags.this.tags
  )
}
...
resource "aws_evidently_feature" "this" {
  ...
  tags = merge(
    var.tags,
    lookup(var.evidently_feature[count.index], "tags"),
    data.aws_default_tags.this.tags
  )
...
resource "aws_evidently_project" "this" {
  ...
  tags = merge(
    var.tags,
    lookup(var.evidently_project[count.index], "tags"),
    data.aws_default_tags.this.tags
  )
...
resource "aws_evidently_segment" "this" {
  ...
  tags = merge(
    var.tags,
    lookup(var.evidently_segment[count.index], "tags"),
    data.aws_default_tags.this.tags
  )
}
...
resource "aws_internetmonitor_monitor" "this" {
  ...
  traffic_percentage_to_monitor = lookup(var.internetmonitor[count.index], "traffic_percentage_to_monitor")

  dynamic "health_events_config" {
    for_each = lookup(var.internetmonitor[count.index], "health_events_config") == null ? [] : ["health_events_config"]
    content {
      availability_score_threshold = lookup(health_events_config.value, "availability_score_threshold")
      performance_score_threshold  = lookup(health_events_config.value, "performance_score_threshold")
    }
  }
}
